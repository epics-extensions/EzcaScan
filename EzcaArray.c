        ...
        for (i=0;i<noName;i++) {
	        ...
	        if (pchan->state == cs_conn && pchan->type != TYPENOTCONN) {
	            ...
	            if (status != ECA_NORMAL) {
		            if (CA.devprflag > 0) 
                    {
                        fprintf(stderr,"Ezca_putArray:  ca_array_put failed on %s!\n", 
                                ca_name(pchan->chid));
                    }               
	                	pchan->error = status;
                }
        ...
        for (i=0;i<noName;i++) {
        	...
        	if (pchan->state == cs_conn && pchan->type != TYPENOTCONN) {
	            ...
	            if (status != ECA_NORMAL) {
		            if (CA.devprflag > 0) 
                    {
                        fprintf(stderr,"Ezca_putArrayEvent:  ca_array_put_callback failed on %s!\n", 
                                ca_name(pchan->chid));
                    }
		            pchan->error = status;
                }
...
struct dbrtemp { chandata *pchan;
	...
	if (CA.devprflag > 0) 
    {
        fprintf(stderr,"Ezca_putTimeCallback: type=%d, count=%d\n",
